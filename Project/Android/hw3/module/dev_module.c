...
int stopwatch_open(struct inode *inode, struct file *file){
	...
	ret=request_irq(irq, inter_handler2, IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, "back", 0);
...
int stopwatch_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos){
	printk("sleep on\n");
	outw(255, (unsigned int)led_addr);
	interruptible_sleep_on(&wq_write);
	outw(4, (unsigned int)led_addr);
	printk("write\n");
	return SUCCESS;
}
